            ...
            if (statusCode == 200) {
                ...
                do {
                    ...
                    guard let home = body["home"] as? Bool else {
                        print("Error: /atHomeBeacon home missing")
                        return
                    }
                    
                    if (home) {
                        guard let callback = self.homeBeaconTimer.userInfo as! (() -> Void)? else {
                            print("Error: no callback to use at home beacon")
                            return
                        }
                        callback()
            ...
            if (statusCode == 200) {
                ...
                do {
                    ...
                    guard let bodyDict = bodyJson as? [String:Any] else {
                        print("Error: allPlayers dict parse failed")
                        return
                    }
                    
                    guard let allPlayersList = bodyDict["all_players"] as? [[String:Any]] else {
                        print("Error: all_players cast to list of dicts failed")
                        return
                    }
                    ...
                    guard let firstTargetId = bodyDict["first_target_id"] as? Int else {
                        print("Error: first_target_id cast to int failed")
                        return
                    }
                ...
                print("/startInfo \(statusCode) failed")
            }
        }.resume()
    }
        ...
        if (self.gameState.isGameOver()) {
            ...
            URLSession.shared.dataTask(with: request) { (data, response, error) in
                ...
                if (statusCode == 200) {
                    ...
                    print("/playerUpdate \(statusCode): unexpected response")
                }
            }.resume()
        }
    }
        ...
        for p in self.gameState.allPlayers {
            for dict in nearbyPlayers {
                guard let id: Int = dict["id"] else {
                    print("Error: no player id for a player in nearbyPlayers")
                    return
                }
                if (p.id == id) {
                    guard let location: Int = dict["location"] else {
                        print("Error: location missing for player \(id)")
                        return
                    }
                    p.zone = location
                    p.nearby = true
